...
from src.a2a.client import A2AClient, TaskResponse
...
# A2A client - increased timeout for complex queries like deep_research
a2a_client = A2AClient(timeout=120.0)
...
async def deep_research(query: str) -> str:
    ...
    """
    # Step 1: Decompose query (Search Agent)
    decompose_result = await call_agent_safe(
        SEARCH_AGENT_URL,
        "decompose_query",
        {"query": query}
    )
    ...
    # Step 2: Search web (Search Agent) - more results
    all_web_results = []
    for q in sub_queries[:3]:
        ...
        all_web_results.extend(result.get("results", []))
    
    # Step 3: Search arXiv (Search Agent) - more papers
    arxiv_result = await call_agent_safe(
        SEARCH_AGENT_URL,
        "arxiv_search",
    ...
    # Step 4: Also search Semantic Scholar for citations data
    semantic_result = await call_agent_safe(
        PAPER_AGENT_URL,
        "semantic_search",
        {"query": query, "max_results": 5}
    )
    semantic_papers = semantic_result.get("papers", [])
    
    # Merge papers (avoid duplicates by title)
    seen_titles = {p.get("title", "").lower() for p in papers}
    for sp in semantic_papers:
        if sp.get("title", "").lower() not in seen_titles:
            papers.append(sp)
            seen_titles.add(sp.get("title", "").lower())
    
    # Step 5: Synthesize findings (Synthesis Agent)
    synthesis_result = await call_agent_safe(
        SYNTHESIS_AGENT_URL,
        "synthesize",
        {
            "query": query,
    ...
    # Step 6: Generate detailed report (Synthesis Agent)
    report_result = await call_agent_safe(
        SYNTHESIS_AGENT_URL,
        "generate_report",
        {
            "query": query,
...
### Deep Research
- deep_research, read_paper_pdf, check_agents_status
"""
